...
type Lesson = { id: number; title: string; icon: string };
...
function LessonsPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        else setCompleted(new Set((data ?? []).map((r) => r.lesson_number as number)));
        setLoading(false);
      });
  }, [userId]);
  ...
  function openLesson(id: number, locked: boolean) {
    if (locked || loading) return;
    navigate({ to: "/lessons/$lessonId", params: { lessonId: String(id) } });
  }
    ...
    <div className="relative min-h-screen overflow-hidden bg-background px-4 pb-32 pt-6">
      ...
      <header className="relative z-10 mx-auto flex max-w-md items-center justify-between px-1 py-2">
        ...
        <div className="flex items-center gap-3 font-pixel text-[9px]">
          ...
          <Link to="/notebook" className="pixel-border-neon bg-card px-2 py-1 text-neon hover:-translate-y-0.5 transition-transform">✎ NOTEBOOK</Link>
      ...
      <section className="relative z-10 mx-auto mt-4 max-w-md pixel-border-neon bg-card/90 p-5">
        ...
        <h1 className="mt-3 font-pixel text-lg text-foreground text-glow leading-tight">UNIT 1:<br />FOUNDATIONS</h1>
        ...
        <div className="mt-4 flex gap-1">
          {LESSONS.map((l) => (
            <div key={l.id} className="h-2 flex-1 border-2 border-border" style={{
      ...
      <section className="relative z-10 mx-auto mt-10 max-w-md">
        <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 400 1600" preserveAspectRatio="none" aria-hidden="true">
          <path
            d="M 200 0 C 80 120, 320 240, 200 360 S 80 600, 200 720 S 320 960, 200 1080 S 80 1320, 200 1440 L 200 1600"
            fill="none" stroke="var(--color-neon)" strokeWidth="3" strokeDasharray="8 12" strokeLinecap="round" opacity="0.55"
          />
        </svg>

        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          {[12, 38, 65, 88].map((top, i) => (
            <div key={i} className="absolute h-3 w-3 rounded-full border border-neon/40" style={{
              top: `${top}%`, left: i % 2 === 0 ? "6%" : "90%", opacity: 0.5,
        ...
        <div className="relative flex flex-col items-center gap-14 py-6">
            ...
            return (
              <div key={lesson.id} className="relative flex flex-col items-center" style={{ transform: `translateX(${offset}px)` }}>
                <LessonNode
                  ...
                  onClick={() => openLesson(lesson.id, isLocked)}
                />
                {isCurrent && (
                  <img src={wizard} alt="Your wizard" width={60} height={60}
          ...
          <div className="relative flex flex-col items-center">
            ...
            <div className={`flex h-24 w-24 items-center justify-center font-pixel text-3xl ${totalDone === LESSONS.length ? "pixel-border-neon" : ""}`}
              style={{
        ...
        <div className="mt-12 text-center">
          <button onClick={resetProgress} className="font-pixel text-[9px] text-muted-foreground hover:text-neon">⟲ RESET QUEST</button>
        </div>
      </section>
    </div>
  );
...
function LessonNode({ lesson, state, onClick }: {
  lesson: Lesson;
  ...
  return (
    <button onClick={onClick} disabled={isLocked} className="group relative z-10 flex flex-col items-center disabled:cursor-not-allowed">
      {isCurrent && (
        <span className="mb-2 bg-neon px-3 py-1 font-pixel text-[9px] text-deep flicker">START</span>
      )}
      ...
      <div className="relative">
        ...
        <div
          className={`flex h-24 w-24 items-center justify-center rounded-full font-pixel text-3xl transition-transform ${isLocked ? "" : "group-hover:-translate-y-1"}`}
          style={{
            background: isDone ? "transparent"
              : isCurrent ? "color-mix(in oklab, var(--color-neon) 22%, var(--color-deep))"
              : "color-mix(in oklab, var(--color-muted) 60%, var(--color-deep))",
            color: isDone ? "var(--color-neon)" : isCurrent ? "var(--color-neon)" : "var(--color-muted-foreground)",
            border: isDone ? "4px solid var(--color-neon)"
              : isCurrent ? "4px solid var(--color-neon)"
              : "3px solid var(--color-border)",
            boxShadow: isCurrent ? "0 0 28px var(--color-neon), inset 0 0 18px color-mix(in oklab, var(--color-neon) 40%, transparent)"
              : isDone ? "0 0 18px color-mix(in oklab, var(--color-neon) 60%, transparent)" : "none",
        ...
        {isDone && (
          <div className="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full font-pixel text-[10px]"
            style={{ background: "var(--color-neon)", color: "var(--color-deep)", border: "2px solid var(--color-deep)" }}
          >✓</div>
        )}
      </div>

      <div className="mt-4 max-w-[220px] text-center">
        <div className={`font-pixel text-[10px] leading-tight ${isCurrent ? "text-neon text-glow" : isDone ? "text-foreground" : "text-muted-foreground"}`}>
          {lesson.title}
        </div>
